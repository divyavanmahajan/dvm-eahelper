"""
SSL and connectivity diagnostics for eahelper.

Run with:  eahelper diagnose [--url URL] [--ca-bundle PATH]
"""
...
def run_diagnostics(leanix_url: str, ca_bundle: str | None = None) -> None:
    ...
    if not system_ok and not custom_ok and not legacy_ok and sys.platform == "win32":
        eahelper_dir = Path.home() / ".eahelper"
        eahelper_dir.mkdir(exist_ok=True)
        win_bundle_path = eahelper_dir / "corporate-ca.pem"
    ...
    if legacy_ok or httpx_legacy_ok:
        ...
        print("\n  Run eahelper with:")
        print("    eahelper proxy --legacy-ssl")
        return

    if custom_ok:
        _ok("SSL verification works with your CA bundle.")
        print(f"\n  Run eahelper with:  --ca-bundle \"{ca_bundle}\"")
        return
  ...
  1. Ask your IT/network team for the corporate root CA PEM file, then:
       eahelper proxy --ca-bundle path\\to\\corporate-root-ca.pem
